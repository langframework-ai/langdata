...
class DatabaseSqlConnectorTest {
  ...
  void initializeclient() {
    Assumptions.assumeTrue(url != null && !url.isEmpty(), "Database url SQL_URL not set");
    Assumptions.assumeTrue(
        username != null && !username.isEmpty(), "Database username SQL_USERNAMEDB not set");
    Assumptions.assumeTrue(
        password != null && !password.isEmpty(), "Database url SQL_PASSWORD not set");
  ...
  void insertRowsTest() throws SQLException {
    ...
    assertTrue(
        dataInserted, "Inserted data ('name': 'test', 'age': 22) not found in the 'test' table");
  }
  ...
  void testAnyQuery() throws SQLException {
    ...
    tableMetadata =
        connectorClient.executeAndPrintQuery(
