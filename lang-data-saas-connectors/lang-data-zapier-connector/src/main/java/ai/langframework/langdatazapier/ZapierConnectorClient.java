...
public class ZapierConnectorClient implements ConnectorClient {
   ...
   *
   * @return a map where the keys are action descriptions (like "Send Email") and the values are the
  ...
  public Map<String, String> listActions() throws Exception {
    ...
    Request request =
        new Request.Builder().url(listActionsEndpoint).addHeader("X-API-Key", accessToken).build();
