...
public class MilvusConnectorClient implements ConnectorClient, VectorInterface {
  ...
  /** Default constructor for MilvusConnectorClient. Initializes the client to null. */
  public MilvusConnectorClient() {
    client = null;
  }

  /**
   * Constructor for MilvusConnectorClient. Connects to Zilliz Cloud using the provided cluster
   * endpoint and token.
   *
  ...
  public MilvusConnectorClient(String clusterEndpoint, String token) {
    ...
    ConnectParam connectParam =
        ConnectParam.newBuilder().withUri(clusterEndpoint).withToken(token).build();

    client = new MilvusServiceClient(connectParam);
  }

  /**
   * Constructs a new Milvus connector client with the specified cluster endpoint, token, embedding
   * client type, and model API key.
   *
   * @param clusterEndpoint The endpoint of the Milvus cluster.
  ...
  public void createCollection(String collectionName, Integer dimension) {
    ...
    CreateSimpleCollectionParam createCollectionParam =
    ...
    if (createCollection.getException() != null) {
      System.err.println(
          "Failed to create collection: " + createCollection.getException().getMessage());
      return;
    }
  ...
  public String describeCollection(String collectionName) {
    ...
    R<DescribeCollectionResponse> collectionInfo =
        client.describeCollection(describeCollectionParam);

    DescCollResponseWrapper wrapperDescribeCollection =
        new DescCollResponseWrapper(collectionInfo.getData());

    return wrapperDescribeCollection.toString();
  }

  /**
   * The `insertEntity` method inserts entities into the specified collection using the Milvus
   * service client. It returns "Inserted Successfully" upon successful insertion.
   *
   * @param rows
   */
  public String insertEntity(List<JSONObject> rows) {
    InsertRowsParam insertRowsParam =
        InsertRowsParam.newBuilder().withCollectionName(collectionName).withRows(rows).build();
  ...
  /**
   * The `SearchANN` method performs an approximate nearest neighbor search in the specified
   * collection using the given query vector and parameters.
   *
   * @param queryVector1
   * @param outputFields
   * @param offset
   * @param limit
   * @return a list of row records representing the search results or an empty list if an error
   *     occurs during the search operation.
   */
  private List<RowRecord> SearchANN(
      List<Float> queryVector1, List<String> outputFields, Long offset, Long limit) {
  ...
  /**
   * The `SearchANN` method performs an approximate nearest neighbor search in the specified
   * collection using the given query vector and parameters. It returns a list of row records
   * representing the search results or an empty list if an error occurs during the search
   * operation.
   *
   * @param queryVector1
   * @param outputFields
   ...
   *     occurs during the search operation.
   */
  public String searchApproximateNearestNeighbor(
      List<Float> queryVector1, List<String> outputFields, Long offset, Long limit) {
    try {
      List<List<Float>> queryVectors = new ArrayList<>();
      queryVectors.add(queryVector1);
  ...
  /**
   * The `SearchWithFilters` method conducts a search in the specified collection using the given
   * query vector and filters. If an exception occurs during the search operation, it prints an
   ...
   * @param limit
   * @param filter
   * @return A list of row records representing the search results, applying pagination if
   *     specified.
   */
  public String searchWithFilters(
      List<Float> queryVector1, List<String> outputFields, Long offset, Long limit, String filter) {
    try {
      List<List<Float>> queryVectors = new ArrayList<>();
      queryVectors.add(queryVector1);
  ...
  /**
   * The `searchWithIds` method retrieves records from the specified collection based on the
   * provided list of IDs. If an exception occurs during the retrieval operation, it prints an error
   * message.
   *
   * @param collectionName
   * @param outputFields
  ...
  /**
   * The `deleteEntities` method removes entities from the given collection using the provided list
   * of IDs. It returns "Successfully deleted" upon success or "Failed to delete entity"if there's
   * an error.
   *
   * @param collectionName
   * @param id
  ...
  public String deleteEntities(String collectionName, List<String> id) {
    ...
    DeleteIdsParam deleteParam =
        DeleteIdsParam.newBuilder().withCollectionName(collectionName).withPrimaryIds(ids).build();
   ...
   *
   * @param collectionName
   * @return "Successfully dropped collection" upon success or prints an error message and returns
  ...
  public String dropCollection(String collectionName) {
    ...
    if (dropCollection.getException() != null) {
      System.err.println(
          "Failed to drop collection: " + dropCollection.getException().getMessage());
    }
  ...
  /**
   * The `getClient` method retrieves the client object associated with the current instance. It
  ...
  /**
   * The `initializeClient` method sets up the client with the specified cluster endpoint and
