...
public class PineconeConnectorClient implements ConnectorClient, VectorInterface {
    ...
    /**
     * Initializes a new Pinecone connector client with the specified apikey, environment,
     ...
     */
    public PineconeConnectorClient(String apiKey, String environment, String projectName, String index, String connectionUrl){
        try{
            connection = new PineconeConnection(
                    new PineconeClientConfig()
                            .withApiKey(apiKey)
                            .withEnvironment(environment)
                            .withProjectName(projectName)
                    , new PineconeConnectionConfig()
                    .withIndexName(index).withConnectionUrl(connectionUrl));
        }
        catch (Exception e){
            e.printStackTrace();
        }
    ...
    /**
     * Initializes a new Pinecone connector client with the specified API key, environment,
     * project name, index, connection URL, type of embedding client, and embedding model API key.
     * @param apiKey The API key for authentication.
     * @param environment The environment for the Pinecone connector client.
     * @param projectName The project name for the Pinecone connector client.
     * @param index The index for the Pinecone connector client.
     ...
     */
    public PineconeConnectorClient(String apiKey, String environment, String projectName, String index, String connectionUrl, EmbeddingClientType embeddingClientType, String modelApiKey){
        try{
            connection = new PineconeConnection(
                    new PineconeClientConfig()
                            .withApiKey(apiKey)
    ...
    public void upsertVectors(List<Float> vectors, String text){
        try {
            Vector v1 = Vector.newBuilder()
                    .setId(new RandomStringGenerator.Builder()
                            .withinRange('0', 'z')
                            .filteredBy(Character::isLetterOrDigit)
                            .build()
                            .generate(3))
                    .addAllValues(vectors)
                    .setMetadata(Struct.newBuilder().putFields("Text", Value.newBuilder().setStringValue(text).build()).build())
                    .build();
